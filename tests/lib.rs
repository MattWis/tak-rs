...
fn all_pieces() {
    ...
    let m = vec!["a2F2", "a1F1", "a3F1", "a4F2", "b2F1", "b1F2", "b4F1", "b3F2",
                 ...
                 "b3F1", "a4R1", "a4F1", "c4L1"];
    play_no_win(m, &mut game);
    play(&("c4F1".parse::<Turn>().unwrap()), &mut game).unwrap();
    assert_eq!(game.check_winner(), Some(Player::One));
}
...
fn full_board() {
    ...
    play_no_win(m, &mut game);
    play(&("d3F2".parse::<Turn>().unwrap()), &mut game).unwrap();
    assert_eq!(game.check_winner(), Some(Player::Two));
}
...
fn convoluted_road_win() {
    let mut game = Board::new(6);
    let m = vec!["a1F1", "b1F1", "c1F1", "d1F1", "d2F1", "d3F1", "c3F1", "b3F1",
                 "b4F1", "b5F1", "c5F1", "d5F1", "e5F1"];
    play_no_win(m, &mut game);
    play(&("f5F1".parse::<Turn>().unwrap()), &mut game).unwrap();
    assert_eq!(game.check_winner(), Some(Player::One));
}
